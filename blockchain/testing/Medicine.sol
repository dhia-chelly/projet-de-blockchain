...
contract Medicine {
    ...
    function receivedMedicine(
        ...
        require(
            _receiverAddr == wholesaler || _receiverAddr == distributor || _receiverAddr == customer,
        ...
        else if(_receiverAddr == customer && status == medicineStatus(5)){
            status = medicineStatus(6);
            emit ShippmentUpdate(address(this), transporters[transporters.length - 1], customer,4, 5);
            return 3;
        }
    }
